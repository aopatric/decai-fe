        ...
        for (let i = 0; i < numItems; i += this.batchSize) {
            ...
            switch (dataType) {
                ...
                case 'labels':
                    ...
                    const labelsAsNumbers = Array.from(labels).map(label => Number(label)); // Convert BigInt to number
                    batch = new ort.Tensor('int64', new BigInt64Array(labelsAsNumbers.map(BigInt)), batchShape); // Convert to BigInt64Array for Tensor
                    break;
            }
